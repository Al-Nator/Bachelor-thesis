...
# Runs 8 Office-Home LODO partial-finetuning trainings for one seed.
#
# Usage:
#   bash scripts/run_large_partial.sh
#   SEED=123 bash scripts/run_large_partial.sh
...
for heldout in "${HELDOUTS[@]}"; do
  for model in "${MODELS[@]}"; do
    pattern="outputs/checkpoints/*_lodo_${heldout}_${model}_partial_finetune_seed${SEED}/best.pt"
    required=""
    if [[ "$model" == *vit* ]]; then
      required="partial_finetune: cls_patch_mean"
    fi
    if run_complete "$pattern" "$required"; then
      echo "skip existing: lodo ${heldout} ${model} partial_finetune seed${SEED}"
      continue
    fi
